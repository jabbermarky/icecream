...
**Superseded as a plan** by the epic (#41) and by `p0.4-batch-schema.md`, which
revised the schema after the codex GATE FAIL. Kept for its reasoning, which is
still the argument the batch loop rests on. Its build order, its `P`/`B`
numbering and its "Phase 0 blocks implementation" gate are all historical —
order lives in the epic.
