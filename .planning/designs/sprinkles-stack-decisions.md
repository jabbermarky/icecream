...
*Status (2026-08-08):* **complete — both oracle sets generated, no remaining gaps.** See `.planning/golden-masters/` (README covers scope, caveats, and the convention mapping between the two engines). The legacy-JS set captures three legacy quirks (fitnessFields order-dependence, Sums NaN on undefined per-ingredient fields, Fitness mutating its target argument). The primary set executes `balance_engine.py` verbatim against the real seed DB (both preserved at `.planning/prototype/`), covering the full formula contract plus the engine's own three recipes with real rows embedded; 18 automated checks against the balance card's printed pistachio-v2 output all pass, and the Mullan known-answer computes PAC 26.21 vs published ~26.9.
