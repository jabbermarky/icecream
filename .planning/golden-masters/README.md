...
| **`balance_engine.py`** (`python-vectors.json`, primary) | ✅ formula vectors generated · ⏳ real-recipe vectors pending seed DB | The *new* formula conventions the TS engine ships with: mass-weighted `balance()`, FPD quadratic, Leighton `frozen_at`, lactose/sandiness derivations, target bands |
...
patch: its hardcoded seed-DB path is redirected to a synthetic ingredient
table (the real `sprinkles-ingredient-seed-db.json` is not yet in this repo).
...
- `recipes` — 4 synthetic-row recipes with full `balance()` output, `assess()`
  marks, fp0, and frozen-%-at-−12°. Internal identities (lactose = MSNF×0.545,
  added sugars, other solids, PAC-excl-lactose) machine-verified.
- `pending_seed_db_expectations` — the balance card's printed values for the
  real pistachio v2 (PAC 19.9, fp₀ −2.1 °C, 74% frozen @ −12 °C, per-ingredient
  PAC contributions…), to be verified the moment the real seed DB lands.

**Synthetic-data caveat:** recipe-level vectors use plausible-but-synthetic
ingredient rows (embedded in the file, so tests are self-contained). PAC
values were seeded from the balance card's contribution column, which is why
the pistachio-v2-shape vector already reproduces the card's PAC/fp₀/frozen-%
almost exactly; composition axes (fat/MSNF) are not card-accurate. When
`sprinkles-ingredient-seed-db.json` is available, regenerate with real rows
and verify the pending expectations.
