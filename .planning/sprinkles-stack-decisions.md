...
- **Account model:** anonymous-first. First visit silently creates a Supabase anonymous session; all data lives in the one cloud schema from day one. A verified email is required at hard gates (first Save, chat), via `linkIdentity()` — the linking flow itself is the verification (D14).
...
**(c) Verified email at hard gates.** Requiring email at first launch would collapse this back into mandatory accounts — the gates go at trigger points instead. Constraint acknowledged: instant start, guaranteed recoverability, and never interrupting the user can't all three hold; recoverability wins at moments of commitment.
- **First explicit Save → hard gate.** D11 defines Save as a deliberate ceremony tied to a real batch; "add an email so this survives your browser" is congruent with that moment. Everything before first Save is frictionless and anonymous. *(Gate placement is the recommended design, not yet user-confirmed.)*
...
- Implementation is nearly free: Supabase's email `linkIdentity()` flow (magic link / OTP) *is* verification — proof of inbox possession by construction.
...
- **Hono deploy target: Supabase Edge Functions vs. standalone Cloudflare Workers.** Recommendation from the second review: default to Edge Functions (Hono runs on Deno) — one platform, one dashboard, one secrets store, no extra CORS origin, auth via the same JWT. Standalone Workers is the fallback if Edge Function limits bite; the specific thing to check is streaming-response support and execution-time limits on long LLM calls. Not locked until that's verified.
- **First-Save email gate placement** — recommended in D14c, not yet explicitly confirmed. Related open design question: **is anonymity a tier or a grace period?** If a grace period (everyone is expected to link eventually), the UI can nag unlinked users freely and the first-Save gate is a backstop. If a legitimate permanent tier, the app must stay pleasant without an identity indefinitely. This decides the tone of every prompt around linking.
...
- **Supabase anonymous sign-in + `linkIdentity()`** — current behavior, including what happens to an anonymous user's rows if cleanup runs before linking, and whether magic-link/OTP linking behaves as assumed (D14b/c). Stated from general knowledge, not verified.
- **Supabase free-tier project pausing.** Free projects pause after inactivity; under D14b the cloud is load-bearing for *everything*, so a paused project means a broken app — for an episodically-used product (nobody churns weekly in January) this may alone justify the paid tier or a keep-alive. Check current policy and price it.
- **Supabase Edge Function limits** — streaming responses and execution time on long LLM calls, before locking the Hono deploy target.
