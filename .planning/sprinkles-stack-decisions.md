...
*Status (2026-08-08):* **both oracle sets are generated** — see `.planning/golden-masters/` (README covers scope, caveats, and the convention mapping between the two engines). The legacy-JS set captures three legacy quirks (fitnessFields order-dependence, Sums NaN on undefined per-ingredient fields, Fitness mutating its target argument). The primary set executes `balance_engine.py` verbatim (now preserved at `.planning/prototype/`) and covers the full formula contract; its recipe-level vectors use synthetic ingredient rows pending the real `sprinkles-ingredient-seed-db.json`, with the balance card's printed pistachio-v2 values recorded as expectations to verify once that lands. **Remaining gap: the seed DB file.**
